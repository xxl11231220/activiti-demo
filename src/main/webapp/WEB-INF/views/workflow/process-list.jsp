		...
		<tbody>
			<c:forEach items="${page.result }" var="object">
				...
				<tr>
					...
					<td><c:choose>
						<c:when test="${!process.suspended}">激活</c:when>
						<c:when test="${process.suspended}">挂起</c:when>
					</c:choose> |
					...
					<td>
                        <a href='${ctx }/workflow/process/delete?deploymentId=${process.deploymentId}' onclick="delcfm()">删除</a>
